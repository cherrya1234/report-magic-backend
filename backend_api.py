...
def df_sample_csv(df: pd.DataFrame, n=20) -> str:
    # small, random sample for grounding
    if len(df) == 0:
        return ""
...
def compute_avg_los(df: pd.DataFrame, size_value="10x10") -> float | None:
    size_col = _pick_col(df, ["UnitSize","Unit Size","Size","Unit_Type","Type"])
    move_in  = _pick_col(df, ["MoveIn","Move In","Move_In","StartDate","Start Date","Start"])
    move_out = _pick_col(df, ["MoveOut","Move Out","Move_Out","EndDate","End Date","End"])
    if not (size_col and move_in and move_out):
        return None
    dfx = df.copy()
    dfx[move_in]  = _parse_date(dfx[move_in])
    dfx[move_out] = _parse_date(dfx[move_out])
    dfx = dfx[dfx[size_col].astype(str).str.strip().str.lower() == str(size_value).lower()]
    if dfx.empty:
        return None
    today = pd.Timestamp(date.today())
    los = (dfx[move_out].fillna(today) - dfx[move_in]).dt.days
    los = los.dropna()
    if los.empty:
        return None
...
# -------- Ask (rewrite + grounded answer + numeric paths) --------
@app.post("/api/ask")
async def ask_question(request: Request):
    try:
        ...
        # Numeric direct compute: average length of stay for 10x10
        text = user_q.lower()
        if "average" in text and ("length of stay" in text or "los" in text) and ("10x10" in text or "10 x 10" in text):
            avg = compute_avg_los(df, "10x10")
            if avg is not None:
                answer = f"The average length of stay for 10x10 units in your data is approximately {avg:.1f} days."
                sess["questions"].append({"question": user_q, "answer": answer})
                return {"answer": answer}

        # Build compact profile + sample for grounding
        profile = df_profile(df)
        sample_csv = df_sample_csv(df, n=20)

        # Rewrite question to analyst brief (internal only)
        rewrite_system = (
        ...
        # Answer grounded in the profile + tiny sample
        answer_system = (
            "You are a precise data analyst. Use ONLY the provided data profile and sample rows to answer. "
            "If information is missing, say what columns or steps are needed. Keep steps short and clear."
        )
        user_context = (
            f"DATA PROFILE\n{profile}\n\n"
        ...
        # Save Q & A (do NOT include brief in export)
        sess.setdefault("questions", [])
        sess["questions"].append({
            "question": user_q,
            "answer": answer,
            # analyst_brief kept out of export intentionally; not stored here to minimize memory
        })
